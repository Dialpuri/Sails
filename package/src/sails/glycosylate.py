...
def glycosylate_xtal(structure: gemmi.Structure | Path | str, mtz: gemmi.Mtz | Path | str, cycles: int, f: str, sigf: str,
...
def xray(args):
    ...
    structure, mtz, log, snfgs = glycosylate_xtal(args.modelin, args.mtzin, cycles, *labels, args.type, args.v)
...
def parse_args():
    ...
    group.add_argument("-modelout", type=str, required=False, default="sails-model-out.cif")
    ...
    group.add_argument("-type", type=Type.from_string, choices=list(Type), default=Type.n_glycosylate)

    formatter = argparse.ArgumentDefaultsHelpFormatter
    xray_parser = subparsers.add_parser("xray", parents=[parent], formatter_class=formatter)
    xray_parser_group = xray_parser.add_argument_group("Required arguments in X-ray mode")
    xray_parser_group.add_argument("-mtzin", type=str, required=True)
    xray_parser_group.add_argument("-mtzout", type=str, required=False, default="sails-refln-out.mtz")
    xray_parser_group.add_argument("-colin-fo", type=str, required=False, default="FP,SIGFP")
