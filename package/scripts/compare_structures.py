...
def main(args):
    ...
    for c in reference[0]:
        for r in c:
            if r.name not in data or gemmi.find_tabulated_residue(r.name).is_amino_acid():
                continue

            centroid = np.mean([a.pos.tolist() for a in r], axis=0)

            nearest_atom = ns.find_nearest_atom(gemmi.Position(centroid[0], centroid[1], centroid[2]))
            nearest_residue = structure[0][nearest_atom.chain_idx][nearest_atom.residue_idx]
